...
<Warning>
  Os **três canais da Meta têm template pré-aprovado**: Meta Cloud API, Instagram e Messenger. O que muda é o escopo, o endpoint e a categoria.

  Hoje os endpoints desta seção cobrem **apenas a Meta Cloud API** (escopo WABA). O suporte a Instagram e Messenger está **em implementação**.
</Warning>
...
|---|---|---|---|
| Canais | Meta Cloud API | **Messenger** | Instagram, Messenger, WhatsApp |
...
<Note>
  Veja [o que cada canal aceita](/channels/message-support).
</Note>
...
<Note>
  Antes de escolher a categoria, veja os [modelos de template](/templates/models/overview): o tipo define quais campos existem, e alguns já vêm com categoria fixa.
</Note>
