...
```bash
curl -X POST https://zapi.omni.z-api.io/channels/SU_CANAL/token/SU_TOKEN/send-text \
  -H "Content-Type: application/json" \
...
<Steps>
  ...
  <Step title="Actualiza la base URL y la autenticación">
    Sustituye `https://api.z-api.io` por `https://zapi.omni.z-api.io` y cambia el header `Client-Token` por `Authorization: Bearer SU_SECRET`.
  </Step>
